...
def load():
    rate = pd.read_csv(os.path.join(PROCESSED, "pterygium_rate_per100k.csv"))
    rate = rate[["year", "prefecture", "setting", "count_per_100k"]]
    out = {}
    for src, (fn, col, _) in UV.items():
        uv = pd.read_csv(os.path.join(BASE, fn))[["year", "prefecture", col]].rename(columns={col: "uv"})
...
if __name__ == "__main__":
    merged = load()
    df_b = pd.read_csv(os.path.join(PROCESSED, "uv_correlation_by_year.csv"))
    plot_yearly_r(df_b)
