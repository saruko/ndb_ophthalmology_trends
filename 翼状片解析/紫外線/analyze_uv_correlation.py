...
UV_SOURCES = {
    "jma_uv": {
        "path": os.path.join(BASE, "jma_uv_index_annual_mean.csv"),
        "value_col": "uv_index_annual_mean",
        "label": "JMA UVインデックス(年平均)",
    },
    "openmeteo_radiation": {
        "path": os.path.join(BASE, "openmeteo_radiation_annual_mean.csv"),
...
def load_merged():
    ...
    df_rate = pd.read_csv(os.path.join(PROCESSED, "pterygium_rate_per100k.csv"))
...
def main():
    ...
    out_a = os.path.join(PROCESSED, "uv_correlation_by_prefecture.csv")
    out_b = os.path.join(PROCESSED, "uv_correlation_by_year.csv")
    ...
    write_summary(df_a, df_b, os.path.join(PROCESSED, "uv_correlation_summary.txt"))
