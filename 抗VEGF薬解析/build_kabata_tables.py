...
先行研究が区別していなかったラニビズマブの先発／バイオシミラーは分離して併記する。
    ...
    python 抗VEGF薬解析/build_kabata_tables.py --nokouhi   # 公費含まない（既定と同じ）
    python 抗VEGF薬解析/build_kabata_tables.py --kouhi     # 親フォルダ（2024年度に公費を含む）

出力先: <データフォルダ>/processed/ → organize_outputs.py で 05_先行研究再現_Kabata/ へ。
他の解析スクリプトと同じく processed/ を経由する（src/paths.py の規約）。
直下に直接書くと organize_outputs.py が「手作業で置いたファイル」と見なして
据え置くため、再実行のたびに同名ファイルが直下に溜まってしまう。

【フラグの向きに注意】本スクリプトの既定は「公費含まない」だが、
extract_*.py / run_antivegf_pipeline.py / analyze_*.py / organize_outputs.py /
build_paper_figures_xlsx.py の既定は**親フォルダ**であり、向きが逆である。
取り違えを防ぐため `--nokouhi` を明示指定することを推奨する
（既定と同じ動作なので、付けても結果は変わらない）。
"""
...
def main():
    ap = argparse.ArgumentParser(
        description="Kabata et al. (2026) の図表を再現するCSVを生成する。")
    # 他スクリプトは --nokouhi 方式（既定＝親フォルダ）だが、本スクリプトの既定は
    # 「公費含まない」である。既定を反転させると、素で実行していた運用が黙って
    # 別データセットを向くため、--nokouhi は「既定の明示」として追加している。
    g = ap.add_mutually_exclusive_group()
    g.add_argument("--nokouhi", action="store_true",
                   help="公費レセプトを含まないデータ（抗VEGF薬解析/公費含まない）で生成する【既定】")
    g.add_argument("--kouhi", action="store_true",
                   help="公費レセプトを含むデータ（抗VEGF薬解析直下）で生成する")
    args = ap.parse_args()

    data_dir = ANTIVEGF_DIR if args.kouhi else os.path.join(ANTIVEGF_DIR, "公費含まない")
    label = "公費レセプトを含む（親フォルダ）" if args.kouhi else "公費レセプトを含まない"
    out_dir = os.path.join(data_dir, "processed")
    os.makedirs(out_dir, exist_ok=True)
    print(f"データセット: {label}")
    print(f"入力: {data_dir}")
    print(f"出力: {out_dir}  （organize_outputs.py で 05_先行研究再現_Kabata/ へ移動）")
    if not (args.kouhi or args.nokouhi):
        print("  ※フラグ未指定のため既定（公費含まない）を使用。"
              "他スクリプトとは既定の向きが逆なので、--nokouhi の明示を推奨")
