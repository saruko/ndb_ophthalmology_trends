...
```bash
python 抗VEGF薬解析/extract_ophthalmic_drugs.py     # 生Excel → CSV抽出（初回のみ）
```

```bash
python 抗VEGF薬解析/extract_g016.py                 # G016算定回数の抽出（初回のみ）
```

```bash
python 抗VEGF薬解析/run_antivegf_pipeline.py        # zero補完で主解析＋感度分析
```

```bash
python 抗VEGF薬解析/analyze_g016.py                 # G016の解析
```

```bash
python 抗VEGF薬解析/analyze_cost.py                 # 医療費の要因分解・治療総額・反実仮想
```
...
```bash
python 抗VEGF薬解析/build_kabata_tables.py --kouhi  # 先行研究の図表再現
```

```bash
python 抗VEGF薬解析/build_censoring_bounds_antivegf.py  # 秘匿識別区間（下限/上限）と収載完全性の検証（査読対応）
```

```bash
python 抗VEGF薬解析/organize_outputs.py             # 出力をフォルダ構成へ振り分け
```
...
**秘匿分の総量は品目×年度×区分ごとに `missing = 公表総計 − Σ開示セル` として厳密に
復元できる**（`build_censoring_bounds_antivegf.py`）。各秘匿セルの真値は `[0, missing]` にあり、
都道府県別・年齢性別の集計はこの下限/上限つきで
`03_解析結果/品質管理_感度分析/prefecture_bounds_antivegf.csv` / `agesex_bounds_antivegf.csv`
に出力している。なお**「秘匿セルは必ず10未満（≦9）」という仮定は成立しない**。
NDBは補完的秘匿を行っており、部分秘匿行に単独秘匿は両軸とも0行、missing が
秘匿セル数×9 を超える行が全部分秘匿行（都道府県軸138行）に及ぶ。
したがって five/random 補完は上限として機能しない（zero は下限として有効）。
全国合計は公表総計ベースのため確定値で、区間幅は総計秘匿行由来の最大0.004%に留まる。
また、上市中の全10製品が上市期間の全年度でNDBに掲載されていることを検証済み
（`listing_completeness_antivegf.csv`。外用薬と異なり上位品目足切りの影響はない）。
