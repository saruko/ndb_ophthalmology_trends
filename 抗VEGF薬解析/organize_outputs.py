...
解析パイプラインを実行したあとに本スクリプトを実行する運用を想定している。
    ...
    python 抗VEGF薬解析/organize_outputs.py --dry-run
    python 抗VEGF薬解析/organize_outputs.py
    python 抗VEGF薬解析/organize_outputs.py --nokouhi
...
def classify(name):
    ...
    if name.startswith(("national_", "trend_")):
