...
def build_manuscript():
    ...
    add_paragraph(
        doc,
        "緑内障手術（K268）の2021→2022年度における急増（35,728→86,207件、+141%）には、"
        "臨床的なMIGS普及に加え、2022年度診療報酬改定に伴う電算処理コードの構造的変化が寄与している。"
        "具体的には、流出路再建術（眼内法）（150435810）および濾過胞再建術（needle法）（150427310）が"
        "2022年度から新たにNDBオープンデータに収載され、これらのコードだけで約39,500件が加算された。"
        "したがって、2022年度以降のK268合算値には、実質的な手術件数の増加と"
        "コード体系の拡大による計上範囲の変化が混在しており、"
        "2021年度以前との単純な件数比較には注意を要する。"
    )
