...
解析パイプラインを実行したあとに本スクリプトを実行する運用を想定している。
    ...
    python 緑内障点眼解析/organize_outputs.py --dry-run     # 主解析（公費含まない）
    python 緑内障点眼解析/organize_outputs.py
    python 緑内障点眼解析/organize_outputs.py --kouhi       # 公費含む版
...
INPUT_FILES = {
    "ndb_processed_glaucoma_zero.csv",
    "ingredient_exposure_glaucoma_zero.csv",
}
# 02_中間データ
INTERIM_FILES = {
    "ndb_glaucoma_age_sex_zero.csv",
...
# 03_解析結果/配合剤_成分別
INGREDIENT_PREFIXES = ("national_ingredient",)
# 03_解析結果/薬効群比較
GROUP_PREFIXES = ("group_", "agent_")
# 03_解析結果/期間別解析（2022年度以降の主解析・収載品目を固定した均衡パネル）
PERIOD_PREFIXES = ("fullcov_", "balanced_panel_")
# 03_解析結果/品目マスタ
INVENTORY_PREFIXES = ("product_", "inventory_summary", "censoring_report",
                      "within_agent_")
# 03_解析結果/代替性分析
SUBSTITUTION_PREFIXES = ("substitution_",)
# 03_解析結果/品質管理_感度分析
QC_PREFIXES = ("censoring_sensitivity", "methods_claims_verification", "coverage_",
               "agesex_completeness", "listing_gaps")
...
def classify(name):
    ...
    if name.startswith(INVENTORY_PREFIXES):
        return os.path.join(RESULT, "品目マスタ")
    if name.startswith(PERIOD_PREFIXES):
        return os.path.join(RESULT, "期間別解析")
    if name.startswith(GROUP_PREFIXES):
        return os.path.join(RESULT, "薬効群比較")
    if name.startswith(INGREDIENT_PREFIXES):
        return os.path.join(RESULT, "配合剤_成分別")
    ...
    if name.startswith("glaucoma_summary_report"):
        return RESULT
    return None
...
EXCLUDE_DIRS = {
    ...
    "提示データ",
    # 原稿ごとに独立した成果物一式（本文md・図表xlsx・専用スクリプト・中間CSV）。
    # 種類別に振り分けると原稿と図表が離れてしまうためフォルダごと据え置く。
    "論文案1",
}
