...
DRUG_CATEGORIES = [
    # 配合剤（FDC）
    # 配合剤の後発品は先発品名を含まない独自の一般名で収載される
    # （ラタチモ＝ザラカム、トラチモ＝デュオトラバ、タフチモ＝タプコム、
    #   ドルモロール＝コソプトの各後発品）。
    ("FDC_PG_BETA",   "PG関連薬・β遮断薬配合点眼（ザラカム等）",  ["ザラカム", "デュオトラバ", "タプコム", "ミケルナ",
                                                                  "ラタチモ", "トラチモ", "タフチモ"]),
    ("FDC_CAI_BETA",  "CAI・β遮断薬配合点眼（コソプト等）",       ["コソプト", "アゾルガ", "ドルモロール"]),
...
# 薬効群の分類
#
# 日本緑内障学会「緑内障診療ガイドライン（第5版）」第5章「緑内障治療薬」1.局所投与薬の
# 分類に準拠する。ガイドライン原典（日眼会誌126巻2号, 令和4年2月10日）で確認した区分は
# 次の9群である。
#
#   1) プロスタノイド受容体関連薬
#        (1) プロスタノイドFP受容体作動薬（ラタノプロスト・トラボプロスト・
#            タフルプロスト・ビマトプロスト）
#        (2) プロスタノイドEP2受容体選択性作動薬（オミデネパグ・イソプロピル）
#   2) 交感神経β受容体遮断薬
#        (1) β受容体非選択性遮断薬（チモロール・カルテオロール・レボブノロール）
#        (2) β1受容体選択性遮断薬（ベタキソロール）
#        (3) α1β受容体遮断薬（ニプラジロール）
#   3) 炭酸脱水酵素阻害薬（局所投与製剤）（ドルゾラミド・ブリンゾラミド）
#   4) 交感神経α2受容体作動薬（ブリモニジン）
#   5) Rhoキナーゼ（ROCK）阻害薬（リパスジル）
#   6) 副交感神経作動薬（ピロカルピン）
#   7) 交感神経α1受容体遮断薬（ブナゾシン）
#   8) イオンチャネル開口薬（イソプロピル・ウノプロストン）
#   9) 配合点眼薬
#
# 注意すべき点（いずれもガイドライン原典で確認済み）:
#   - ウノプロストンは「イオンチャネル開口薬」であり、プロスタノイド受容体関連薬には
#     含まれない（BKチャネル開口による線維柱帯流出路からの房水流出増加）。
#   - ニプラジロールは独立群ではなく「交感神経β受容体遮断薬」の下位分類(3)である。
#   - オミデネパグはプロスタグランジン骨格を持たないが、プロスタノイドとして分類され
#     「プロスタノイド受容体関連薬」の下位分類(2)に置かれる。
#
# ガイドラインに記載のない薬剤（ジピベフリン・ジスチグミン）は OTHER_NON_GL として
# 別群にまとめる。緑内障の適応を持ちNDBに収載されるため全体合計には含める。
# アプラクロニジンはレーザー術前後の一過性眼圧上昇予防薬として第6章に記載され、
# 眼圧下降薬の9群には含まれないため、本解析でも別掲とし全体合計から除く。
#
# 配合点眼薬は複数の薬効群にまたがるため、単剤の薬効群とは重複させず独立群として扱う。
# ─────────────────────────────────────────────

# 1) プロスタノイド受容体関連薬
PGA_FP_CODES  = {"LATANOPROST", "TRAVOPROST", "TAFLUPROST", "BIMATOPROST"}
PGA_EP2_CODES = {"OMIDENEPAG"}
PGA_CODES     = PGA_FP_CODES | PGA_EP2_CODES

# 2) 交感神経β受容体遮断薬（ニプラジロール＝α1β受容体遮断薬を下位分類として含む）
BETA_NONSEL_CODES = {"TIMOLOL", "CARTEOLOL", "LEVOBUNOLOL"}
BETA_B1SEL_CODES  = {"BETAXOLOL"}
BETA_A1B_CODES    = {"NIPRADILOL"}
BETA_CODES        = BETA_NONSEL_CODES | BETA_B1SEL_CODES | BETA_A1B_CODES

CAI_CODES          = {"DORZOLAMIDE", "BRINZOLAMIDE"}  # 3) 炭酸脱水酵素阻害薬
ALPHA2_CODES       = {"BRIMONIDINE"}                  # 4) 交感神経α2受容体作動薬
ROCK_CODES         = {"RIPASUDIL"}                    # 5) Rhoキナーゼ阻害薬
PARASYMPATHO_CODES = {"PILOCARPINE"}                  # 6) 副交感神経作動薬
ALPHA1_CODES       = {"BUNAZOSIN"}                    # 7) 交感神経α1受容体遮断薬
ION_CHANNEL_CODES  = {"UNOPROSTONE"}                  # 8) イオンチャネル開口薬
FDC_CODES   = {"FDC_PG_BETA", "FDC_CAI_BETA", "FDC_A2_BETA",
               "FDC_A2_CAI", "FDC_ROCK_A2"}           # 9) 配合点眼薬

# ガイドライン第5版に記載のない緑内障適応薬
OTHER_NON_GL_CODES = {"DIPIVEFRINE", "DISTIGMINE"}

# 慢性緑内障治療の総量。アプラクロニジンは短期使用のため含めない。
TOTAL_CODES = (PGA_CODES | BETA_CODES | CAI_CODES | ALPHA2_CODES
               | ROCK_CODES | PARASYMPATHO_CODES | ALPHA1_CODES
               | ION_CHANNEL_CODES | FDC_CODES | OTHER_NON_GL_CODES)

# 成分別曝露量の集計（配合剤を成分に分解する）。カテゴリ間で重複するため、
# シェア・HHI・総量の計算には使わず、別ファイルに出力する。
INGREDIENT_MAP = {
    "ING_PGA":    ("プロスタノイド受容体関連薬（成分ベース：配合剤を含む）",
                   PGA_CODES | {"FDC_PG_BETA"}),
    "ING_BETA":   ("β遮断薬（成分ベース：配合剤を含む）",
                   BETA_CODES | {"FDC_PG_BETA", "FDC_CAI_BETA", "FDC_A2_BETA"}),
    "ING_CAI":    ("炭酸脱水酵素阻害薬（成分ベース：配合剤を含む）",
                   CAI_CODES | {"FDC_CAI_BETA", "FDC_A2_CAI"}),
    "ING_ALPHA2": ("α2受容体作動薬（成分ベース：配合剤を含む）",
                   ALPHA2_CODES | {"FDC_A2_BETA", "FDC_A2_CAI", "FDC_ROCK_A2"}),
    "ING_ROCK":   ("ROCK阻害薬（成分ベース：配合剤を含む）",
                   ROCK_CODES | {"FDC_ROCK_A2"}),
}

# 薬効群の集計。ガイドライン第5版 第5章 1.局所投与薬 の9群の順序に従う。
# 互いに重複せず、合計＝GLAUCOMA_EYE_TOTAL。
GROUP_DEFS = [
    ("PGA",           "プロスタノイド受容体関連薬",   PGA_CODES),
    ("BETA",          "交感神経β受容体遮断薬",        BETA_CODES),
    ("CAI",           "炭酸脱水酵素阻害薬",           CAI_CODES),
    ("ALPHA2",        "交感神経α2受容体作動薬",       ALPHA2_CODES),
    ("ROCK",          "Rhoキナーゼ阻害薬",            ROCK_CODES),
    ("PARASYMPATHO",  "副交感神経作動薬",             PARASYMPATHO_CODES),
    ("ALPHA1",        "交感神経α1受容体遮断薬",       ALPHA1_CODES),
    ("ION_CHANNEL",   "イオンチャネル開口薬",         ION_CHANNEL_CODES),
    ("FDC_TOTAL",     "配合点眼薬",                   FDC_CODES),
    ("OTHER_NON_GL",  "その他（ガイドライン記載外）", OTHER_NON_GL_CODES),
]
# 下位分類（上位群と重複するためシェア・HHIの計算には使わない）
SUBGROUP_DEFS = [
    ("PGA_FP",       "プロスタノイドFP受容体作動薬",   PGA_FP_CODES),
    ("PGA_EP2",      "プロスタノイドEP2受容体作動薬",  PGA_EP2_CODES),
    ("BETA_NONSEL",  "β受容体非選択性遮断薬",          BETA_NONSEL_CODES),
    ("BETA_B1SEL",   "β1受容体選択性遮断薬",           BETA_B1SEL_CODES),
    ("BETA_A1B",     "α1β受容体遮断薬",               BETA_A1B_CODES),
]
