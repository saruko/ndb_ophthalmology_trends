"""抗アレルギー点眼薬解析の入出力フォルダ構成と、公費レセプトの有無の切り替え。

抽出データ・中間データ・解析結果・図表・論文成果物が同じ階層に混在しないよう、
下記の構成で管理する（抗VEFG薬/src/paths.py と同じ方針）。
...
詳細は 05_論文成果物/公費含まない/README.md を参照。
...
def data_root(nokouhi):
    ...
    主解析（公費含まない）は allergy解析/ 直下、公費含む版は 公費含む/ の下。
    """
...
def raw_file(stem, year, nokouhi, agesex=False):
    ...
    （2014〜2023年度はもともと公費含まない集計であり、差し替え不要）。
        ...
        stem: "ndb_gaiyo" / "ndb_chusha" / "ndb_gaiyo_agesex" / "ndb_chusha_agesex"
    """
