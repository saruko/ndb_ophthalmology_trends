...
def analyze_trends_and_apc(df, output_dir):
    """抗アレルギー点眼薬の全国トレンドおよびAPCを計算して保存する"""
    print("Analyzing national trends and APC for anti-allergy eye drops...")
    ...
    df_apc.to_csv(os.path.join(output_dir, "national_apc_linear_allergy.csv"), index=False, encoding="utf-8-sig")
    
    if joinpoint_results:
        df_jp = pd.DataFrame(joinpoint_results)
        df_jp.to_csv(os.path.join(output_dir, "national_apc_joinpoint_allergy.csv"), index=False, encoding="utf-8-sig")
        
    df_national.to_csv(os.path.join(output_dir, "national_trends_allergy.csv"), index=False, encoding="utf-8-sig")
...
def analyze_geographic_disparity(df, output_dir):
    ...
    print("Analyzing geographic disparity (CV & Gini) for anti-allergy eye drops...")
    ...
    df_disp.to_csv(os.path.join(output_dir, "geographic_disparity_allergy.csv"), index=False, encoding="utf-8-sig")
    print("Saved geographic_disparity_allergy.csv")
    return df_disp

def analyze_covariates_correlation(df, output_dir):
    """高齢化率、眼科専門医数、眼科施設数とのSpearman相関を算出する"""
    print("Analyzing correlation with covariates for anti-allergy eye drops...")
    ...
    df_corr.to_csv(os.path.join(output_dir, "covariates_correlation_allergy.csv"), index=False, encoding="utf-8-sig")
...
def run_panel_regression(df, output_dir):
    ...
    for code, group in df.groupby("code"):
        ...
        try:
            ...
            report_path = os.path.join(output_dir, f"panel_regression_{code}_allergy_report.txt")
...
def analyze_allergy_all(processed_csv_path, output_dir):
    """すべてのアレルギー統計解析を実行するエントリーポイント"""
    ...
    print("All allergy analyses completed successfully!")
