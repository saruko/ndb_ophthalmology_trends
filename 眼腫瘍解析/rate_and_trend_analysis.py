...
眼腫瘍手術トレンド_2014_2024.csv（都道府県別集計由来、秘匿の影響が小さい真の全国算定回数）を用いる。
年齢・性別クロス集計は個々のセルが10件未満で頻繁にマスクされるため、
そこから再合算した値は低頻度術式ほど過小評価される（extract_age_stratified.pyの整合性チェックで確認済み）。

入力:
  01_抽出データ/眼腫瘍手術トレンド_2014_2024.csv （都道府県別集計由来の真の全国合計、年次）
  01_抽出データ/人口_4群層別化_2014_2024.csv （4群人口、年次）
出力:
  processed/rate_per_100k_national_trend.csv   （全年齢人口10万対rateの年次推移）
  processed/poisson_trend_test_results.csv     （術式別 Poisson回帰トレンド検定結果）
  ※ organize_outputs.py で 03_解析結果/ へ振り分けられる
"""
import os
import sys
import numpy as np
...
# 観測期間が2022-2024年の3年分のみ。11年間のtrend testの対象としては不適切なため
# 参考値（改定前後の比較）にとどめ、Poisson回帰の対象からは除外する。
NEW_CODE_MIN_YEARS = 2022

# K225（結膜腫瘍冷凍凝固術）は2022年度にK225-4が新設されたことで
# 2023年に算定回数が急減（前年比-73%）しており、2014-2024の11年を単一線形モデルで
...
def main():
    df_total = pd.read_csv(find("眼腫瘍手術トレンド_2014_2024.csv"))
    df_pop = pd.read_csv(find("人口_4群層別化_2014_2024.csv"))
    ...
    rate_out = os.path.join(OUT_DIR, "rate_per_100k_national_trend.csv")
    ...
    # 粗数トレンドとrateトレンドの方向一致チェック（2014->2024の増減方向）
    print("\n=== 粗数 vs rate トレンド方向の一致チェック（2014年→2024年） ===")
    consistency_rows = []
    ...
    for code, g in df_rate.groupby("k_code_group"):
        ...
        if code == "K225-4":
            results.append({
                "k_code_group": code, "apc_percent": np.nan, "apc_ci_low": np.nan,
                "apc_ci_high": np.nan, "p_value": np.nan, "n_years": len(g),
                "note": f"2022年度診療報酬改定でコード新設のため{NEW_CODE_MIN_YEARS}-2024年の3年分のみ。"
                        "11年trend testの対象外（改定前後の参考値のみ算出可）",
            })
            continue
        if code == "K225":
            # 2022年度K225-4新設により2023年に算定回数が急減（前年比-73%）しており、
            # 11年通しの単一線形Poissonモデルは構造的断絶を均すため不適切。
            ...
            res["note"] = (
                ...
                "2022年以降はK225-4への症例移行が関連する可能性。"
                "11年通しの単一線形回帰は断絶を均すため採用しない。"
            )
            results.append(res)
            continue
        if g["count"].sum() == 0:
    ...
    trend_out = os.path.join(OUT_DIR, "poisson_trend_test_results.csv")
