...
OUTPUT_PATH = os.path.join(output_dir(), "文書1.docx")
...
def main():
    ...
    run_title = p_title.add_run("レセプト情報・特定健診等情報データベース（NDB）オープンデータを用いた日本における眼腫瘍関連手術の全国トレンド解析（2014〜2024年度）")
    ...
    run_title_en = p_title_en.add_run("An 11-Year Nationwide Trend Analysis of Ophthalmic Tumor Surgeries in Japan Using the National Database of Health Insurance Claims Open Data (2014–2024)")
    ...
    add_abstract_section(doc, "背景", "日本の超高齢社会において、眼腫瘍手術の全国的トレンドや年齢層別動向は十分に明らかにされていない。本研究は、レセプト情報・特定健診等情報データベース（NDB）オープンデータを用いて、過去11年間の主要な眼腫瘍関連手術の動向を包括的に解釈することを目的とした。")
    add_abstract_section(doc, "方法", "2014〜2024年度のNDBオープンデータから、眼瞼結膜腫瘍、眼窩腫瘍、眼球摘出、眼内腫瘍に関する16の手術電算コードを抽出した。分母として総務省の全国人口を用い、全年齢の人口10万対粗算定率（Crude Rate）を算出した。年間パーセント変化（APC）および傾向検定は、対数人口をオフセット項としたPoisson回帰モデルを用いて算出した。")
    add_abstract_section(doc, "結果", "11年間で、眼瞼結膜悪性腫瘍手術（K216: APC = +1.16%, p = 0.009）や眼窩腫瘍摘出術（K234: APC = +5.92%, p < 0.001）は有意な増加傾向を示した。一方、眼球摘出術（K241: APC = -5.84%, p < 0.001）は有意な減少を示した。結膜腫瘍冷凍凝固術（K225）は2014〜2021年度にかけて急増していたが（APC = +10.58%, p < 0.001）、2022年度の「角結膜悪性腫瘍切除術（K225-4）」新設に伴い、2023年度に件数が73%減少（463件→123件）し、2024年度にはさらに63件へと減少する構造的断絶が認められた。")
    ...
    p4.add_run("本研究は、2014年度から2024年度までの11年間のNDBオープンデータを用いて、日本における主要な眼腫瘍関連手術の算定動向を体系的に明らかにし、人口動態および治療技術の進歩、診療報酬改定が与えた影響について多角的に考察することを目的とした。")

    # 4. 対象と方法 (Methods)
    add_heading_styled(doc, "2. 対象と方法（Methods）", level=1)
    
    p5 = add_paragraph_styled(doc)
    p5.add_run("本研究は、2014年度（第1回）から2024年度（第11回）までのNDBオープンデータにおける「手術（都道府県別）」および「手術（性・年齢階級別）」の集計ファイルを使用した。また、分母データとなる分母人口には、総務省統計局が公表している「人口推計（各年10月1日現在）」の全国男女別・各歳別人口データを用いた。なお、国勢調査実施年である2015年および2020年については、前後年のデータから線形補間した値を使用した。")
    ...
    add_paragraph_styled(doc, "表1: 各術式のPoisson傾向検定結果（2014〜2024年度）", align=WD_ALIGN_PARAGRAPH.LEFT)
    ...
    for _, row in df_res.iterrows():
        ...
        if pd.isna(p_val):
            ...
            # 表示桁数は全解析共通（README §4.3）: p値は3桁、0.001未満は <0.001
            p_str = f"{p_val:.3f}"
    ...
    p10.add_run("主要な術式における算定回数および人口10万対算定率の期首・期末の比較を表2に示す。眼瞼結膜悪性腫瘍手術（K216）は2014年度の397件（10万対0.31）から2019〜2023年度には500件前後まで増加した後、2024年度には300件（10万対0.24）へ減少したが、11年間を通したPoisson回帰ではAPC = +1.16%（p = 0.009）と有意な増加傾向が認められた。また、眼窩内腫瘍摘出術（表在性）（K234）は1,338件（10万対1.05）から2023年度の2,423件（10万対1.95）へとほぼ倍増し、2024年度も2,222件（10万対1.79）と高水準を維持する極めて顕著な増加（APC = +5.92%, p < 0.001）を示した。")
    
    p11 = add_paragraph_styled(doc)
    p11.add_run("一方、眼球摘出術（K241）は2014年度の200件（10万対0.16）から2024年度には49件（10万対0.04）へと有意に減少した（APC = -5.84%, p < 0.001）。また、結膜腫瘍冷凍凝固術（K225）は、2014年度の317件から2021年度には604件（10万対0.48）へと有意に増加（APC = +10.58%, p < 0.001）していたが、2022年度の「角結膜悪性腫瘍切除術（K225-4）」新設を経て、2023年度には123件（対2022年比73%減）、2024年度にはさらに63件へと急減する構造的断絶が確認された。")

    # 表2の作成
    add_paragraph_styled(doc, "表2: 主要術式の算定数と人口10万対算定率の推移（2014年度 vs 2024年度）", align=WD_ALIGN_PARAGRAPH.LEFT)
    
    table2 = doc.add_table(rows=1, cols=6)
    table2.alignment = WD_TABLE_ALIGNMENT.CENTER
    set_table_borders(table2)
    
    headers2 = ["Kコード", "術式名", "2014年度 算定数 (10万対率)", "2024年度 算定数 (10万対率)", "変化率 (%)", "備考"]
    ...
    # 主要術式データの抽出
    # 2014年と2024年の比較
    rates_csv = find("rate_per_100k_national_trend.csv")
    ...
    for tk in target_k:
        ...
        val_2024 = df_tk[df_tk['year'] == 2024].iloc[0]
        
        c_2014 = int(val_2014['count'])
        r_2014 = val_2014['count_per_100k']
        c_2024 = int(val_2024['count'])
        r_2024 = val_2024['count_per_100k']
        
        pct_change = ((c_2024 - c_2014) / c_2014) * 100
        
        t2_row = table2.add_row().cells
        t2_row[0].text = tk
        t2_row[1].text = code_names.get(tk, tk)
        t2_row[2].text = f"{c_2014:,} ({r_2014:.2f})"
        t2_row[3].text = f"{c_2024:,} ({r_2024:.2f})"
    ...
    p14.add_run("まず、眼球摘出術（K241）が11年間で有意に減少（APC = -5.84%, p < 0.001）している点は、臨床における眼球温存療法の普及を強く示唆している。特に小児の眼腫瘍（網膜芽細胞腫など）や成人の眼内腫瘍（脈絡膜悪性黒色腫など）に対して、従来は眼球摘出術が標準治療であったが、近年は放射線療法（小線源治療や重粒子線治療）、化学療法（超選択的眼動脈注入療法など）といった集学的治療の発展により、眼球や視機能を温存する選択率が高まっている。例えば、Abramsonらは、眼動脈注入化学療法（OAC）の導入により、進行性の片側性網膜芽細胞腫に対する一次眼球摘出率が95%以上から7.4%へと劇的に低下したことを単一施設の後方視的検討から実証している[2]。日本国内においても同様に、温存療法の普及が手術件数の減少というトレンドとしてマクロに現れていると解釈するのが合理的である。")
    
    p15 = add_paragraph_styled(doc)
    p15.add_run("次に、眼瞼結膜悪性腫瘍手術（K216）が有意な増加傾向（APC = +1.16%, p = 0.009）を示したことは、日本の超高齢社会の進展と密接に関連している。眼瞼に発生する悪性腫瘍のうち、脂腺癌や基底細胞癌は高齢者に好発することが知られている。Satoらの日本国内における眼瞼悪性腫瘍の包括的なレビューによれば、本邦においては基底細胞癌に次いで脂腺癌の頻度が他国に比して高く、その多くが60歳以上の高齢者に発生する[3]。今回の年齢階級別解析において、K216の算定件数の88%〜100%が高齢者（65歳以上）で占められていた事実は、高齢人口の絶対的増加が悪性腫瘍手術件数の底上げにつながっている現状を明瞭に示している。")
    
    p16 = add_paragraph_styled(doc)
    p16.add_run("また、眼窩内腫瘍摘出術（表在性）（K234）が極めて著明な増加（APC = +5.92%, p < 0.001）を示した背景には、画像診断技術（高解像度MRIやCT）の普及・高度化による無症候性あるいは早期の眼窩腫瘍（涙腺腫瘍、海綿状血管腫など）の発見率向上があると考えられる。さらに、内視鏡手術手技や手術ナビゲーションシステムの臨床導入といった手術支援技術の向上により、以前は手術困難とされた深在性あるいは複雑な眼窩腫瘍に対しても安全にアプローチできるようになり、適応が拡大していることも算定件数の増加に寄与していると推察される。")
    ...
    p20.add_run("2014〜2024年度の11年間にわたるNDBオープンデータを用いた解析により、日本の眼腫瘍関連手術は、眼球温存療法の普及を反映した眼球摘出術の減少、高齢化に伴う眼瞼悪性腫瘍手術の増加、診断・手術支援技術の進歩を背景とした眼窩腫瘍手術の増加など、臨床現場の実態を色濃く映し出した多様な動向を示した。また、結膜腫瘍冷凍凝固術の推移が示すように、診療報酬点数表の改定（新規コード新設）に伴う症例再分類の影響を考慮する重要性が示された。これらのマクロデータは、今後のわが国における眼腫瘍診療体制の整備や医療政策の評価において、価値ある統計的エビデンスを提供するものである。")
