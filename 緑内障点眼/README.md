...
```bash
python 緑内障点眼/run_glaucoma_pipeline.py
```

```bash
python 緑内障点眼/build_product_inventory.py
```
...
```bash
python 緑内障点眼/run_censoring_sensitivity.py
```

```bash
python 緑内障点眼/build_paper_outputs.py
```

```bash
python 緑内障点眼/organize_outputs.py
```

```bash
python 緑内障点眼/add_rounded_columns.py
```
...
| 4 | `run_paper_analyses.py` | 主解析（2022–2024）・均衡パネル・成分内シェア・年齢調整 | 1・2の出力 |
...
| 5 | 薬効群（ガイドライン分類）の群間比較 | `src/analysis_group_comparison.py` |
...
（11年度通算149品目）。分類漏れの確認は、131かつ「点眼」を含む未分類品目名を
目視で走査して行った。
...
**日本緑内障学会「緑内障診療ガイドライン（第5版）」の眼圧下降点眼薬の分類**に準拠。
`GROUP_DEFS`（10群・互いに排他）と `SUBGROUP_DEFS`（下位分類・上位群と重複）で管理する。

| 群コード | 薬効群 | 下位分類 |
|---|---|---|
| `PGA` | プロスタグランジン関連薬 | `PGA_FP` / `PGA_EP2` / `PGA_ION` |
| `BETA` | 交感神経β遮断薬 | `BETA_NONSEL` / `BETA_B1SEL` |
| `ALPHA1BETA` | 交感神経α1β遮断薬 | — |
| `ALPHA1` | 交感神経α1遮断薬 | — |
| `ALPHA2` | 交感神経α2刺激薬 | — |
| `SYMPATHO` | 交感神経非選択性刺激薬 | — |
| `PARASYMPATHO` | 副交感神経刺激薬 | `PARASYM_DIR` / `PARASYM_IND` |
| `CAI` | 炭酸脱水酵素阻害薬 | — |
| `ROCK` | Rhoキナーゼ阻害薬 | — |
| `FDC_TOTAL` | 配合点眼薬 | — |
...
- **アプラクロニジン（アイオピジンUD）は全体合計に含めない**。レーザー後等の一過性眼圧上昇に
  対する短期使用薬であり、慢性緑内障治療の処方量と性質が異なるため別掲とする。
...
`ING_BETA` にはニプラジロール（α1β遮断薬）も含める（β遮断作用を持つため）。
...
- 都道府県別ファイル: **1,595行で不一致0件**
- 年齢性別ファイルを含む照合: **3,138行で不一致0件**
...
```bash
python 緑内障点眼/run_glaucoma_pipeline.py --imputation upper --output-dir 緑内障点眼/processed_upper
```
...
ガイドライン分類の10群について、年度ごとに以下を並べて比較する。
...
2021年度の30品目から2022年度の137品目へ増加しており、**2021年度以前と2022年度以降の
...
### 6-11. 都道府県比較の年齢調整（`src/analysis_age_adjusted.py`）
...
### 6-12. 論文成果物の生成（`build_paper_outputs.py`）
...
### 6-13. 年齢×性別（`build_glaucoma_age_sex.py`, `src/analysis_age_sex_rates.py`）
...
| 項目 | allergy解析 | 本解析 |
|---|---|---|
| 単位 | 統一せず素合算 | 全品目をmLに統一換算（`count_ml`）。判定規則を1,595行で検証 |
| 薬効分類 | 3クラス（抗ヒスタミン／メディエーター遊離抑制／免疫抑制） | 緑内障診療ガイドライン第5版の10群＋7下位分類 |
